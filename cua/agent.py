...
- **GenerateImage(requirement)**: Generate an SVG image (icon, illustration, diagram). Uses multi-round generation + visual self-review. Returns PNG file path. Use to remember window positions, icon locations, file paths, or task progress. Notes appear automatically in think(). Call with no text to read all notes.
